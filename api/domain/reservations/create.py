...
def create_reservation(
    ...
    if existing is not None:
        return _replay(existing, req.tenant_id, zone, now)
    ...
    session.commit()
    return _draft(reservation, zone, now)
...
def _replay(
    existing: Reservation, tenant_id: uuid.UUID, zone: ZoneInfo, now: datetime
) -> ReservationDraft:
    if existing.tenant_id != tenant_id:
        raise Conflict("Idempotenz-Schlüssel gehört zu einem anderen Vorgang")
    return _draft(existing, zone, now)


def _draft(r: Reservation, zone: ZoneInfo, now: datetime) -> ReservationDraft:
