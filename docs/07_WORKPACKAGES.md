...
| T-1.8 | #29 | Tool `transfer_to_team` inkl. Schleifenschutz und Erreichbarkeitsprüfung | 04 | T-1.1 | fertig 17.09.2026: `domain/callbacks/transfer.py` + `tools/transfer_to_team.py`, Zeilensperre und Zustand auf `calls.transfer_reason` gegen Mehrfachauslösung je Anruf, Erreichbarkeit aus Öffnungszeiten (Annahme, siehe docs/01), 15 Tests, p95 deutlich unter 300 ms |
