...
| T-4.6 | #50 | Übergabe über n8n mit `handover_state`: **B** Netzwerk-Bondrucker (ESC/POS) zuerst, **C** <Kassensystem>-Bestell-Eingang als zweiter Adapter nach Antwort von <Kassenanbieter>; Idempotenz, Wiederholung | 02 §2, 01 D2 | T-1.6 | B fertig 25.09.2026: Druckbruecke statt Push ueber n8n, weil weder Server noch n8n den Drucker im Lokal erreichen, ohne den Router zu oeffnen (docs/02 §2a). `domain/ordering/handover.py` (Abholen mit 60-s-Leihe, Rueckmeldung, ueberholte Revision wird nie ausgeliefert, `handover_state` folgt nur der neuesten Revision, Waechter: 60 s unabgeholt macht die Karte rot, Alarm, `order.handover_failed`), `api/kitchen/router.py` (`/v1/kitchen/claim`, `/ack`, eigenes Token), `api/jobs/cold_path.py` (Dispatcher plus Waechter), `printbridge/` (ESC/POS PC858, TCP 9100 mit Statusabfrage oder Windows-Warteschlange, Druckprotokoll gegen doppelte Zettel). Backoff-Regeln nach `events/outbox.py`, gemeinsam fuer Dispatcher und Bruecke. 44 neue Tests, `/v1/kitchen/claim` p95 5,8 ms. Keine Migration. Offen: echter Probedruck vor Ort, Rechner fuer die Bruecke; **C** blockiert bis Antwort <Kassenanbieter>. **Neu 26.09.2026 (D2 entschieden, docs/02 §2a):** Bons druckt die Kasse; die Bruecke druckt bis zur Schnittstelle jede Bestellung als Eingabezettel mit Kopf "NICHT IN KASSE – bitte eingeben" auf den Haupt-Bondrucker an der Kasse (80 mm), danach nur als Notweg. Offen dafuer: Kopfzeile im Bon, Druckziel, Wortlaut von `handover_state` in der GUI |
| T-4.7 | #51 | GUI Spalte „Neue Bestellungen" mit Passt/Korrigieren und Korrekturgründen | 06 §3 | T-3.1, T-4.5 | fertig 25.09.2026: `domain/ordering/board.py` (Spalte: unabgehakte Bestellungen des Betriebstags plus jede wartende und jede rote Karte ohne Tagesgrenze; `approve_order` setzt `approved` und schickt ausserhalb von `primary` den Bon; `resend_order` fuer "Nochmal senden"), `correction.py` (Menge, Tauschen mit Hinweis, Dazunehmen per Kartennummer, Pflichtauswahl nie vorbelegt, Summe neu ueber die gespeicherten Positionen, Hinweis faellt nur ausdruecklich weg, Stand-Pruefung und doppelter Tap, `order.corrected` ohne Personendaten), `ticket.py` (Bon mit `revision`, ueberschreibt einen nie versuchten statt ihn zu ueberholen), `labels.py` (Namen je Position inkl. Korrektur). GUI: Karten, Passt, Nochmal senden, Korrektur-Kasten ausserhalb der Liste, Ereignis `orders` im Strom, Ton und Blinken. Vor dem Bau Design-Review aus drei Blickwinkeln, 12 bestaetigte Befunde eingebaut. Keine Migration (PR #139 belegt 003). Eigenes Review PR #140 (6 Befunde behoben: kein "KORREKTUR" nach fehlgeschlagener Uebergabe, Mengengrenze des Agenten, kein Editor fuer stornierte Bestellungen, Namensliste mit Notbehelf statt Fehler 500, Speichern ohne zweite Vorschau, Tippen haelt den Kasten offen). 37 neue Tests, im Browser durchgeklickt, Spalte p95 unter 300 ms |
| T-4.8 | #52 | GUI „Gericht aus" | 06 §3 | T-4.1 | fertig 26.09.2026: `domain/menu/sold_out.py` (Liste mit Suche, Schalter bis Ende des Betriebstags mit Zeilensperre und `audit_log`, Zahl und Fingerabdruck für Kachel und Strom, bis zu zwei Alternativen derselben Kategorie), `search_menu` nennt sie im `say`; GUI: Knopf in der Kopfzeile mit Zahl, Kasten mit Suchfeld und Schaltern, Ereignis `dishes`. Keine Migration (`sold_out_until` seit 002). Im Browser durchgeklickt, Schalter p95 unter 300 ms |
| T-4.10 | - | Wünsche zu Positionen: Gericht und Wunsch im selben Satz trennen („die 23 ohne Karotten" findet heute nichts), Wunsch als Option der Karte (Preis aus `item_options`, Aufpreis beim Wiederholen genannt) oder als Hinweis nur für Weglassen („ohne", „kein"); alles, was nicht auf der Karte steht, bietet der Agent nicht an. Begründung für einen Aufpreis nur aus der Karte (neue optionale Spalte im Options-Import, Migration). Allergie ist kein Wunsch: Hinweis an die Küche im festen Wortlaut „WICHTIG: Keine <Zutat>. Grund: Allergie" (E14) plus Allergenpfad aus T-4.4, keine Zusage | 04, 05, 14 | T-4.5 | fertig 24.09.2026: Allergie im festen Wortlaut E14 als Hinweis; `domain/menu/wishes.py` (`split_wish`, `classify_wish`), `search_menu` liefert `wish` (note, option mit Aufpreis und Grund, allergy ohne Zusage, unknown nicht angeboten, open bei mehreren Treffern); Wiederholung mit Aufpreis im Gesprächskern; Migration 003 `item_options.price_reason` mit optionaler Importspalte; Schlüssel aus der geprüften Anfrage (offene Punkte PR #127); Latenz gemessen |
| T-4.11 | - | Umwandler Kasse → Importformat: liest Kopien von `artikel.DBF`/`.DBT`, `zutaten.DBF`/`.DBT`, `warengrp.dbf`, `zutgrp.DBF` nur lesend und schreibt die CSV-Dateien nach 14. Preis aus `VK1_PREIS`, Kategorie aus `W_BEZEICH`, Größen als Pflichtgruppe, Zutaten als freiwillige Optionen, Allergene nur über die Umsetztabelle a–n → LMIV; Platzhalterzeile `000` raus, abgeschnittene Namen, abweichender `VK2_PREIS` und Allergen-Lücken im Bericht, Artikelnummer der Kasse in Originalschreibweise als eigenes Feld `menu_items.pos_code` (Migration); im Export fehlende oder gesperrte Artikel werden nach `--dry-run` per Schalter inaktiv | 14 §Quelle Kasse | T-4.2, die sechs Kassendateien | fertig 26.09.2026: `domain/menu/pos_dbf.py` (eigener Leser, dBase IV mit Memo, cp437 aus dem Kopf, nur lesend), `domain/menu/pos_convert.py`, `scripts/kasse_to_csv.py`; Größe n kostet VK1 + GRPREIS(n-1), Namen 1 normal, 2 klein, 3 groß, 4 party aus der Maske; Extras über `WRGSHOW` und Preisstufe `ZPREIGRP3`; keine Sperr-Spalte, gesperrt = gelöscht markiert; Warengruppen ohne Telefonbestellung per `--skip-groups`; Migration 004 `pos_code`, Import mit optionaler Spalte `pos_code` und `--deactivate-missing`. Echte Dateien: 123 Gerichte, 751 Optionen; 50 Gerichte (`25G`, Sushi `S1` … `SM6`) nicht übernommen, weil die Suche die Nummern nicht versteht (eigene Aufgabe) |
...
| T-5.1 | #54 | Eval-Runner `evals/runner.py`, Fall-Format, Report | 08 | T-4.5 | fertig 25.09.2026: `evals/runner.py` (Wegwerf-Datenbank je Lauf, Evalkarte `evals/menu/`, Fälle über `sim/replay.py`, Tag-Filter, Exit 0/1/2), `judge.py` (Abgleich mit dem Datenbankzustand, unbekannte Erwartung bricht ab), `recorder.py` (geratene Position, `confirm` ohne Ja, gebucht ohne `confirm`), `report.py` (JSON und Markdown, Regressionsregel gegen den letzten Lauf), `scratch_db.py` (auch für `conftest.py`). Neuer Fall `eskalation_0001`. 4/4 Fälle grün, harte Metriken 0, Suite läuft in CI. `eval_runs` offen (Migration nach PR #139), Tokens mit T-2.4 |
| T-5.2 | #55 | Eval-Suite v1: mindestens 100 Fälle (Menü, Mengen, Optionen, Störgeräusche, Eskalation) | 08 | T-5.1 | fertig 26.09.2026: 107 Faelle in `evals/cases/` (Abholung 57, Reservierung 26, Eskalation 16, Allergie 5, Lieferung 3, davon vier aus T-5.1 und einer aus T-4.10), jede Pflichtzeile aus docs/08 §6 belegt, `api/tests/test_evals_suite.py` liest die Tags aus der Tabelle in §6. Runner: `sold_out` je Fall, `pending` als bekannte Luecke mit Aufgabe (CI: ohne pending gruen, mit pending rot). Evalkarte mit Allergenen. Stand Skript-Modell 99/107, harte Metriken 0, falsche Eskalation 4,5 %, Lauf 12 s. Zwei Fehler im Skript-Modell behoben (Ja auf die einzige Rueckfrage, Nummer im ersten Satz als Rufnummer), gefunden: `check_slot` bietet Zeiten des Vortags als Alternativen an (eigener Fix), Nummernregel bei "und die 24" und "ich wuerde die 13" (behoben in PR #147, seitdem 101/107) |
