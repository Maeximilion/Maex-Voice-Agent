...
| T-4.4 | #48 | Tool `get_item_details` inkl. Allergen-Regel „unbekannt ≠ keine" | 04 | T-4.2 | fertig 22.09.2026: `domain/menu/details.py` + `tools/get_item_details.py`; Allergene nur aus `item_allergens`, ohne Zeile `known: false` und der Satz fürs Team aus dem Code, Codes in LMIV-Reihenfolge, `confirmed_at` als Datum; Optionen und „heute aus" über den gemeinsamen Baustein `domain/menu/items.py`, den auch `search_menu` benutzt; p95 ~16 ms |
| T-4.5 | #49 | Tool `draft_order` mit allen Prüfungen und `readback`; dabei den Satz je Position zerlegen, bevor `search_menu` gefragt wird - Regel A macht aus „die 23 und einmal Pho Bo" sonst eine Rückfrage statt zweier Positionen (`docs/01_STATUS.md` § Open Points, `docs/04` §search_menu) | 04 | T-4.3, T-4.4 | offen |
