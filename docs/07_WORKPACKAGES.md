...
| T-4.3 | #47 | Tool `search_menu` mit der Auflösungsreihenfolge aus 04, Trigram-Index, Schwellen konfigurierbar | 04 | T-4.2 | fertig 18.09.2026: `domain/menu/search.py` + `api/tools/search_menu.py`; Reihenfolge Nummer, Alias, Trigram über Name und Alias, Schwellen aus `MENU_FUZZY_THRESHOLD_HIGH`/`_LOW`, Füllwörter fallen vor dem Vergleich weg, mehrdeutig heißt nachfragen (max. 3), inaktive Gerichte unsichtbar, ausverkaufte sichtbar mit `sold_out`; p95 ~15 ms |
| T-4.4 | #48 | Tool `get_item_details` inkl. Allergen-Regel „unbekannt ≠ keine" | 04 | T-4.2 | fertig 18.09.2026: `domain/menu/details.py` + `api/tools/get_item_details.py`; Allergene nur aus `item_allergens`, ohne gepflegte Zeile `known: false` und der Satz fürs Team aus dem Code, Codes in LMIV-Reihenfolge, `confirmed_at` als Datum; p95 ~16 ms |
