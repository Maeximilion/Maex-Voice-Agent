...
- `expected` kennt `intent`, `confirmed`, `escalated`, `items` (`number`, `quantity`, optional `options` und `note` im festen Wortlaut, etwa der Allergiehinweis aus E14), `customer_name`, `party_size` und `tools` (Tools, die das Modell aufgerufen haben muss: eine Allergiefrage gilt nur mit `get_item_details` als beantwortet, T-5.2). Verglichen wird nur, was im Fall steht. Ein unbekannter Schlüssel, ein fehlendes Feld oder eine doppelte `id` bricht den Lauf mit Exit 2 ab, statt still grün zu sein.
