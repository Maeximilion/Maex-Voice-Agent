...
- Eval suite v1 (T-5.2): 105 cases in `evals/cases/` covering every mandatory line of docs/08 §6 (the table there now carries the tag each line needs, checked by `api/tests/test_evals_suite.py`); cases can set `sold_out` and mark a known gap with `pending` plus the task that closes it; CI requires every other case green and every gap red. The eval menu now has allergens for 23 and 24
