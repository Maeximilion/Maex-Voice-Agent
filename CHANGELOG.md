...
- Tool `POST /v1/tools/get_item_details` (T-4.4): description, options and allergens for a `menu_item_id` from `search_menu`; without a maintained row in `item_allergens` there is no allergen statement, only `known: false` and the sentence that the team calls back, and maintained codes come in the order of the LMIV list
